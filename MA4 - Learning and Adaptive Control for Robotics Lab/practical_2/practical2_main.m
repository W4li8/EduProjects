...
% Add some obstacles
myWorld.addCylinder(0.2, [-0.4, -0.4, 0]);
%myWorld.addEllipsoid([0.1, 0.1, 0.2], [0, 0.2, 0.5]);
%myWorld.addPlane([0.1, 0.1, 0.2], [0, 0, 0]);
% Plot some isolines
isolines = linspace(1, 1.4, 5);
adjust_scale = 1;
myWorld.showGammaIsosurfaces(isolines, adjust_scale);
...
function xdot = modulatedDS(x, attractor, world, nominalDS)
    ...
    for k = 1:kObst   
        ...
        D = eye(3) + diag([-1, 1, 1])./gamma(k);
    
        % Compute normal and tangent on ellipse
        normal = world.listOfObstacles(k).gradientGamma(x(1), x(2), x(3));
        tangent1 = [-normal(2); normal(1); normal(3)];
        tangent2 = cross(normal, tangent1);%[normal(3); normal(2); -normal(1)];
