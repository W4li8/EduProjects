...
# from attackedmodels.WIMP import wimp_test
...
class SceneAugmentor:
    def __init__(self, argo_path=get_argo_val_path(), ckpt_path=get_lanegcn_path(), correct_speed=True, render=False, models_name="LaneGCN"):
        ...
        if models_name == "LaneGCN":
            ...
            # load pretrained model
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            self.WIMP_obj = wimp_test.WIMP_argoverse_dataset(argo_path = argo_path)
            path = get_wimp_path()
    ...
    def get_WIMP_agent_pred(self):
        ...
        """
        print(self.scenario_idx, self.data['orig'], self.data['rot'], self.attack_params, self.avl.seq_list[self.scenario_idx])
        ...
        self.all_preds = denorm_preds.copy()
