...
from attackedmodels.UberLaneGCN.SocialNCE.contrastive import SocialNCE
...
config["hist_size"] = 20  # see l.168 in data.py for this random division
config["pred_size"] = 30  # see l.168 in data.py for this random division
config["pred_step"] = 1
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        #! contrastive loss -> SocialNCE setup
        self.nce = SocialNCE(config)
        self.nce_input = None
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        nodes_a2m = self.a2m(nodes, graph, actors, actor_idcs, actor_ctrs)
        nodes = self.m2m(nodes_a2m, graph)
        actors_m2a = self.m2a(actors, actor_idcs, actor_ctrs, nodes, node_idcs, node_ctrs)
        actors = self.a2a(actors_m2a, actor_idcs, actor_ctrs)
        ...
        #! copy data for social nce (note: at first this was a clone of actors_m2a)
        ## regression:    out['reg'] --> BATCH_SIZE (32), N_AGENTS, N_MODS (6), TRAJ_PRED (30), XY (2)
        ## classification: out['cls'] --> BATCH_SIZE (32), N_AGENTS, N_MODS (6)
        self.nce_input = torch.cat([reg[:, 0, :, :].clone() for reg in out["reg"]]).float().cuda()  # (0 is most likely trajectory)
        #* continue (end of edits)
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, data: Dict, net, am) -> Dict:
        ...
        #! adding nce loss and weighting of loss functions
        loss_nce = net.nce.contrastive_loss(data=data, feat=net.nce_input, argo_map=am)

        w_mse = 10  # weight for the original loss (version of mse)
        w_nce = 1   # weight for contrastive loss (cross entropy)

        # easy toggle of contribution print to log
        if True:
            a = w_mse*loss_out["loss"].item()
            b = w_nce*loss_nce.item()
            print('loss_out', a, 'loss_nce', b, 'contrib %', int(b/(a+b)*100))

        # weigh losses and make sure it is retained in params linked to model
        loss_out["loss"] = w_mse*loss_out["loss"] + w_nce*loss_nce
        #* continue (end of edits)
...
class SynthesizedArgoDatasetSmallAll(Dataset):
    def __init__(self, config, size=10000):
        self.config = config
        self.size = size
        with open(get_syn_data_train(), "rb") as f:  # rb: read binary
