...
class Message:
    ...
    def decode(self):
        ...
        if self.id == Message.ID_DESCRIPTION:
            ...
            self.version, offset = self.get_uint16(0)
        elif self.id < Message.ID_BOOTLOADER_RESET:
            self.params = []
            offset = 0
            while offset < len(self.payload):
                param, offset = self.get_uint16(offset)
                if param >= 1<<15:
                    param -= 1<<16
...
class Thymio:
    ...
    def handle_message(self, msg):
        ...
        if msg.id == Message.ID_NODE_PRESENT:
            ...
            pass  # ignore
        elif msg.id < Message.ID_BOOTLOADER_RESET:
            with self.callback_lock:
                if msg.id in self.callbacks and callable(self.callbacks[msg.id]):
    ...
    def send_event(self, eventId, args):
        """Send an event to the Thymio"""
        payload = Message.uint16array_to_bytes(args) if isinstance(args, list) else args
        msg = Message(eventId, 0, payload)
        self.send(msg)

    def register_callback(self, eventId, callback):
        """Register a callback to be called when the Thymio emits a custom event.
           Note that the callback will be called in a different thread."""
        with self.callback_lock:
